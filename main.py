...
FUNKCJE
    ...
    * program_start() - inicjalizuje główne okno, konfigurację i uruchamia pętlę mainloop
...
def program_start():
    ...
    global label_wyniki_klienci, label_wyniki_leki
    ...
    def dodaj_klienta():
        ...
        sukces=customer.add_customer(name=imie,email=email,phone=telefon,street=ulica,city=miasto,country=kraj)
        if sukces:
            messagebox.showinfo("Sukces",f"Zarejestrowane pacjenta {imie}")
            pokaz_statystyki()
        else:
            messagebox.showerror("ERROR","Nie udało się dodać pacjenta")
    ...
    def usun_klienta():
        ...
        try:
            ...
            sukces=customer.remove_customer(id=k_id,name=None)
            if sukces:
                messagebox.showinfo("Sukces",f"Usunięto klienta o ID: {k_id}")
                pokaz_statystyki()
            else:
                messagebox.showerror("ERROR","Nie znaleziono klienta o takim ID")
        except Exception:
            messagebox.showerror("Error","Wpisz poprawne dane!")
    ...
    def kup():
        ...
        Pobiera tekstowe ID klienta, nazwę leku oraz żądaną ilość. Blokuje transakcję,
        ...
        try:
            ...
            recepta = entry_recepta.get().strip()
            if kup_id_str == "ID klienta" or lek_nazwa_input == "Nazwa leku" or ile_str == "Ilość":
                messagebox.showerror("Błąd", "Wypełnij najpierw wszystkie pola formularza zakupu!")
                return
            kup_id = int(kup_id_str)
            ile = int(ile_str)
            koszyk = [{'amount': ile, 'drug_name': lek_nazwa_input, 'prescription': recepta}]
            wyniki = customer.customer_buy_drug(kup_id, koszyk)
            if wyniki and wyniki[0]:
                messagebox.showinfo("Sukces",f"Zrealizowano zakup!\nLek: {lek_nazwa_input}\nIlość: {ile} szt.")
                pokaz_statystyki()
            else:
                messagebox.showerror("Odrzucono","Zakup odrzucony! Sprawdź czy:\n1. ID Pacjenta istnieje\n2. Lek jest w magazynie\n3. Ilość nie przekracza stanu")
        except ValueError:
